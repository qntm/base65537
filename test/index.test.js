...
describe('base65537', () => {
  describe('encode', () => {
    it('works', () => {
      assert.deepEqual(encode(Uint8Array.from([])), '')
      assert.deepEqual(encode(Uint8Array.from([0])), '㐀')
    })
  })

  describe('decode', () => {
    it('works', () => {
      assert.deepEqual(decode(''), Uint8Array.from([]))
      assert.deepEqual(decode('㐀'), Uint8Array.from([0]))
      assert.deepEqual(decode('💩'), Uint8Array.from([255, 0]))
      assert.deepEqual(decode('💩💩'), Uint8Array.from([255, 1, 255, 1]))
      assert.deepEqual(
        decode('💩💩💩💩💩💩💩💩💩💩💩💩💩💩💩💩💩💩💩💩💩💩'),
  ...
  describe('demo', () => {
    it('works', () => {
      assert.deepEqual(encode(md5), '䯕傺檸𥴏髌𡎱𦸜💩')
      assert.deepEqual(decode('䯕傺檸𥴏髌𡎱𦸜💩'), md5)
    })
  })
  ...
  describe('round trips', () => {
    ...
    uint8Arrays.forEach((uint8Array, i) => {
      it(`case #${i}`, () => {
        assert.deepEqual(decode(encode(uint8Array)), uint8Array)
      })
    })
  })
})
